...
;(function ( $, window, document, undefined ) {
		...
		$.extend(Plugin.prototype, {
			...
			fetchWeather: function () {
				...
				var lat = this.settings.lat;
				var lon = this.settings.lon;
				console.log(lat+" - "+ lon);
				...
				if (this.settings.api == "openweathermap") {
					...
					//console.log(this.settings.lang);
					parameters.lang = this.settings.lang;
			...
			render : function (weather, context) {
				...
				//console.log(context.settings.days);
				...
				if (context.settings.view != "simple") {
					...
					if (context.settings.view != "partial") {
						...
						$("<li/>").html(context.settings.max + weather.today.temp.max + "<sup>" + degrees + "</sup>").appendTo(temp);
						$("<li/>").html(context.settings.min + weather.today.temp.min + "<sup>" + degrees + "</sup>").appendTo(temp);
		...
		*/
		function datamapper (input, settings) {
			...
			if (settings.api == "openweathermap") {
				...
				out.today.day = getDayString(new Date(), settings.days);
				
				out.forecast = [];
				for (var i = 0; i < settings.forecast; i++) {
					var forecast = {};
					forecast.day = getDayString(new Date(input[1].list[i].dt * 1000), settings.days); //api time is in unix epoch
		...
		function getDayString(date, days) {
		  return days[date.getDay()]; //['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][date.getDay()];
		};
