  ...
  function CarrelloAjax() {
    ...
    if($carrello[0]['suggerimenti_sui_prezzi']==1) { ?>
      ...
      g_table._insert(function(){
        <?php 
        $art=retArticoloFromCat($this->getCurrent());
        ...
        $("form.rsTable2-insert-form-ecommerce_offerte").append("<input type='hidden' value='<?php echo $this->getCurrent(); ?>' name='<?php echo $off_id_cat[0]['id']; ?>' />");
    ...
    <?php 
    $tmpcurrentid=$this->getCurrent();
    if($tmpcurrentid==-1) $tmpcurrentid="";
    ?>
    
    if(typeof(rsCAjax_<?php echo $tmpcurrentid; ?>)=="undefined") eval("var rsCAjax_<?php echo $tmpcurrentid; ?>='';");
    
    function ecomm_initCartAjax_<?php echo $tmpcurrentid; ?>() {
      if(rsCAjax_<?php echo $tmpcurrentid; ?>=="") {
        rsCAjax_<?php echo $tmpcurrentid; ?>=1;
        var tmpCart_<?php echo $tmpcurrentid; ?> = new CarrelloAjax('<?php echo $tmpcurrentid; ?>','<?php echo $this->getCombi(); ?>',"<?php echo ln('Prodotto aggiunto al carrello'); ?>.<br><a href='carrello.html'><?php echo ln('Vai al carrello'); ?></a> <?php echo ln('oppure'); ?> <a class='crt-result-close-a' href='#'><?php echo ln('Continua lo shopping'); ?></a>","<?php echo ln('Modifiche salvate correttamente.'); ?><br><a href='<?php echo getCurLanClass(); ?>/carrello.html'><?php echo ln('Vai al carrello'); ?></a> <?php echo ln('oppure'); ?> <a class='crt-result-close-a' href='#'><?php echo ln('continua lo shopping'); ?></a>","<?php echo $info_carello[0]['prodotti_consigliati']; ?>");
        tmpCart_<?php echo $tmpcurrentid; ?>.initAjax();
      }  
    }
    
    $(document).ready(function(){
      ecomm_initCartAjax_<?php echo $tmpcurrentid; ?>();  
    }); 
    </script>
    <?php
    $this->g_jsCode.=ob_get_contents();  
  ...
  function stampaCarrello() {
    ...
    while (list($key, $nodo) = each($ramo)) {
      ...
      while (list($key2, $tmpcar) = each($tmp_arr)) {
        ...
        $car_list['is_for_mockup'] = $tmpcar['is_for_mockup']; 
    ...
    <div class="rsLoading">Loading...</div>
    <!-- Plain box -->
    <div class="ez-wr ecomm_carrello ecomm_carrello_<?php echo $this->g_current; ?> ecomm-carrello container-fluid">
    	<div class="separated">
      <!-- Module 3A -->
      <div class="ez-wr row separated">
        <?php 
        $notZoom=true;
        ...
        <div class="ez-fl ez-negmr ez-33 crt-col-two crt-articolo-descrizione <?php echo $col_class; ?>"  >
          ...
          <div class="ez-wr">
            ...
            <!-- Plain box -->
            <div class="ez-wr crt-caratteristiche-container">
              <?php
              while (list($key, $tmp_caratteristiche) = each($ecommerce_caratteristiche)) {
                ...
                $tmpArrMockup=array();
                while (list($ml_k, $ml_v) = each($mockup_list)) {
                    array_push($tmpArrMockup, $ml_v['id_ecommerce_mockup']);      
                }
                $tmp_caratteristiche['is_for_mockup'] = implode(";", $tmpArrMockup);
                ...
                <div class="ez-wr crt-caratteristica row crt-label-container-car-<?php echo $tmp_caratteristiche['id']; ?>" caratteristica="<?=$tmp_caratteristiche['id']?>" obblig="<?=$tmp_caratteristiche['obbligatorio']?>" nome="<?=$tmp_caratteristiche['nome']?>" >
                  ...
                  <div class="ez-last ez-oh col-sm-8 ">
                    <!-- Module 3A -->
                    <div class="ez-wr">
                      <div class="ez-fl  ez-negmx ez-33 crt-caratteristiche-input bts-caratteristiche-input" style="">
                        <div class="ez-box" >
                          <?php
                          if($tipo=="1") {
                          ...
                          if($tipo=="9") { 
                            $mockupSettings=retMockupArray($ecommerce_valori[0]['Note_textarea']);
                            ?>
                            <!-- Module 2A -->
                            <div class="ez-wr">
                              <div class="ez-fl ez-negmr ez-50 crt-upload-cmyk-l">
                                <div class="ez-box">
                                  <form id="crt-frm-upload-cmyk" >
                    			     <div id="crt-queue"></div>
                                     <img class="crt-lbl-upload-cmyk" src="<?php echo $objUtility->getPathBackofficeResources().'folder.png'; ?>"/>
                                     <input id="crt-upload-cmyk" name="crt-upload-cmyk" type="file" multiple="false" caratteristica="<?=$tmp_caratteristiche['id']?>" class="file_upload-btn" >
                    			  </form>
                                </div>
                              </div>
                              <div class="ez-last ez-oh crt-upload-cmyk-r">
                                <div class="ez-box">
                                  <img class="crt-upload-cmyk-cancel" src="<?php echo $objUtility->getPathBackofficeResources().'upload_cancel1.png'; ?>" />
                                </div>
                              </div>
                            </div>
