...
class ConfigTool
{
    ...
    if( $this->isDefined( $pKey ) )
		...
		if($val=="" && $pKey=="path-virtual-root") {
      $tArr=explode("\\", dirname(__FILE__));
      if(count($tArr)==1) {
        $tArr=explode("/", dirname(__FILE__));
        array_pop($tArr);
        $vroot=implode("/", $tArr);
      ...
      $val = "/".str_replace($_SERVER['DOCUMENT_ROOT'], "", $vroot);
      $val=str_replace("//", "/", $val);
    }
	...
	function setConfigFromFile( $pFileName )
	{
		...
		{
			// if the filename is not specified, tries to load default config file
			$this->FILENAME = SERVER_DOCROOT . "logic/config_ini.php";
		}
...
require_once (SERVER_DOCROOT."logic/rsConfig.php");
?>