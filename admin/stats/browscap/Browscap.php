...
class Browscap
{
	 ...
	 */
	public $remoteIniUrl	= 'http://browsers.garykeith.com/stream.asp?Lite_PHP_BrowsCapINI';
...
//	public $remoteVerUrl	= 'http://updates.browserproject.com/version-date.asp';
	...
	public $timeout			= 5;
	public $updateInterval	= 604800;		// 7 days
	public $errorInterval	= 7200;			// 2 hours
	//public $doAutoUpdate	= true;	Variabile prelevata dal config.php - vedi riga 218
	...
	{	
		...
		if (!$this->_cacheLoaded) {
			...
			$this->doAutoUpdate = $GLOBALS['bcap_auto_update'];		/*** ***/
...
//			throw new Browscap_Exception("Bad datetime format from {$this->remoteVerUrl}");
			$this->_logErrors("Bad datetime format from {$this->remoteVerUrl}");
		}
...
//				throw new Browscap_Exception('Your server can\'t connect to external resources. Please update the file manually.');
	...
	private function _logErrors($error)
	{
		if (defined('__PHP_STATS_PATH__'))
			$logfname = __PHP_STATS_PATH__.'php-stats.log';
		else
			$logfname = 'php-stats.log';
		file_put_contents($logfname, "BROWSCAP|".date("d/m/Y H:i:s")."|$error\n", FILE_APPEND);
	}
}
