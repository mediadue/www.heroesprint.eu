...
/*** ORIGINALE
$mode=(isset($_GET['mode']) ? (int)addslashes($_GET['mode']) : ($modulo[4]<2 ? 1 : 0));
***/
...
//query data,visits
//mode 1:
//query: data, visits of current month
//array format:
//Array(
//        Array((string)query,(int)visits),
//        ...
//);

//(array)$data_previousWords:
//mode 1:
//query: data, visits of last month
//array format:
//Array(
//        Array((string)query,(int)visits);
//        ...
...
do{
        ...
        while($row=mysql_fetch_row($res)){
                ...
                $data_words[]=Array($query_data,(int)$query_visits);
        }
        $dataFound=true;

        //creation of $data_previousWords
        if($mode!==0) break;//no division by month
        ...
        while($row=mysql_fetch_row($res)){
                list($query_data,$query_visits)=$row;
                $data_previousWords[]=Array($query_data,(int)$query_visits);
        }
...
//(array)$processed_words:
//Array(
//        Array((string)word,(int)visits,(int)previousvisits,(float)increment)
//        ...
//);
...
if($dataFound){
        ...
        if($previousDataFound){
                ...
                foreach($tmp_words as $word => $visits){
                        ...
                        else{ $increment=null; $prev=null; }

                        $processed_words[]=Array($word,$visits,$prev,$increment);
                }
        }
        else{
                foreach($tmp_words as $word => $visits) $processed_words[]=Array($word,$visits,null,null);
        }
}
...
//(array)$output_words:
//Array(
//        Array((string)word,(int)visits,(string)level,(string)altimage)
//        ...
//);
...
if($dataFound){
        for($i=0,$tot=count($processed_words);$i<$tot;++$i){
                ...
                if($increment===null){ $level='new'; $alt_img=''; }
                else{
                        if($increment<-15) $level='1';
                        elseif($increment<-5) $level='2';
                        elseif($increment<5) $level='3';
                        elseif($increment<15) $level='4';
                        else $level='5';
                ...
                $output_words[]=Array($word,$visits,$level,$alt_img);
        }
...
if($mode===0) $phpstats_title=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($selected_month),$selected_year),$string['searched_words_title_2']);
...
if($dataFound){
        ...
        if($mode===0){
                ...
                for($i=0,$tot=MIN(count($output_words),100);$i<$tot;++$i){
                        ...
                        "\n\t<td bgcolor=$style[table_bgcolor] nowrap width=\"16\"><span class=\"tabletextA\"><img src=\"templates/$option[template]/images/icon_level_$level.gif\" title=\"$alt_img\" alt=\"\"></span></td>".
                        "</tr>";
                }
//                $return.="<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"4\" nowrap></td></tr>";
        }
...
//                $return.="<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"3\" nowrap></td></tr>";
        ...
        if($mode===1) $return.=info_box($string['information'],$error['searched_words']);
                ...
                $tmp=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($selected_month),$selected_year),$error['searched_words_2']);
...
if($modulo[4]==2){
        ...
        if($mode==0){
                ...
                $return.=
                "<br><center>".
                "<FORM action='./admin.php?action=searched_words&amp;mode=$mode&amp;group=$group' method='POST' name=form1>".
                "&nbsp;<span class=\"tabletextA\">$string[calendar_view]</span><SELECT name=selected_month>";// MONTH SELECTION
        ...
        $return.=
        "<tr>\n<td><span class=\"testo\"><a href=\"admin.php?action=searched_words&amp;mode=$mode&amp;group=$new_group{$dateSelected}\">".
        "<img src=\"templates/$option[template]/images/icon_changevis.gif\" border=\"0\" align=\"middle\" hspace=\"1\" vspace=\"1\" alt=\"\">";
...
function processWords($data_array,$min_characters){
        ...
        for($i=0,$tot=count($data_array);$i<$tot;++$i){
                ...
                for($x=0,$totx=count($query);$x<$totx;++$x){
                        ...
                        if(strlen($word)<=$min_characters) continue;

                        if(isset($processed_words[$word])) $processed_words[$word]+=$visits;
                        else $processed_words[$word]=$visits;
                }
        }
...
{
      $words[] = array(strtolower($key),$value);//array di record
...
while(count($toSearch)>0)
{
   ...
   for($i=1,$tot=count($toSearch);$i<$tot;++$i)//e la controllo con tutte le altre indicate dall'array toSearch
   {
      ...
      else $newToSearch[]=$toSearch[$i];//parola diversa, la metto nell'array della prossima ricerca
   }
