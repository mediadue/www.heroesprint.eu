...
if($from == '') $from = '0.0.0.1';
if($to == '') $to = '255.255.255.255';
...
if($num_totale>0)
  ...
  $return.=
  "<br><table border=\"0\" $style[table_header] width=\"550\" align=\"center\" class=\"tableborder\"><tr>".
  "<td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center></center></span></td>".
  draw_table_title($string['ip'],"ip","admin.php?action=ip&amp;from=$from&amp;to=$to",$tables,$q_sort,$q_order).
  draw_table_title($string['ip_last_visit'],"data","admin.php?action=ip&amp;from=$from&amp;to=$to",$tables,$q_sort,$q_order).
  draw_table_title($string['ip_hits'],"accessi","admin.php?action=ip&amp;from=$from&amp;to=$to",$tables,$q_sort,$q_order).
  draw_table_title($string['ip_visite'],"visite","admin.php?action=ip&amp;from=$from&amp;to=$to",$tables,$q_sort,$q_order).
  draw_table_title($string['ip_tracking']).
  "</tr>";
  ...
  while($row=mysql_fetch_array($result))
    ...
    $return.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '$style[table_hitlight]', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\"><td width=\"5\" bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$current</span></td><td bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">";
        ...
        $return.=
    "</span></td><td bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">".formatdate($row[1])." - ".formattime($row[1])."</span></td>".
  ...
  if($numero_pagine>1)
    {
    $return.=
    "<tr><td bgcolor=$style[table_bgcolor] colspan=\"6\" height=\"20\" nowrap>".
    pag_bar("admin.php?action=ip&sort=$sort&order=$order&amp;from=$from&amp;to=$to",$pagina_corrente,$numero_pagine,$rec_pag);
//    "<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"6\" nowrap></td></tr>";
  ...
  "<tr><td bgcolor=$style[table_bgcolor]><input name=\"export_choice\" type=\"hidden\" value=12><center><input type=\"submit\" value=\"$pref[start_export]\"></center></td></tr>".
  "</table>".
  "</form>";
  }
else
  {
  $return.=
  "<span class=\"pagetitle\">$phpstats_title<br></span><br>".
  "<table border=\"0\" $style[table_header] width=\"500\" align=\"center\">".
  "<tr><td bgcolor=$style[table_title_bgcolor]><center><span class=\"tabletitle\">$string[cercaip_result] $from a $to </span></center></td></tr>".
  "<tr bgcolor=\"#B3C0D7\"><td bgcolor=$style[table_bgcolor]><center><span class=\"tabletextA\">".
  ($check_range===true ? '' : "$string[cercaip_check]<br>").
  "$string[cercaip_noresult]</span></center></td></tr>".
  "<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"6\" nowrap></td></tr></table>";

//$return.=info_box($string['information'],$error['ip']);
  }

  // CERCAIP by Dapuzz
  if($from == '0.0.0.1') $from = '';
  if($to == '255.255.255.255') $to = '';
  $return.=
  "<br><form name=\"cercaip\" action=\"admin.php?action=ip\" method=\"get\">".
  "<table border=\"0\" $style[table_header] width=\"416\" align=\"center\">".
  "<tr><td bgcolor=$style[table_title_bgcolor]><span class=\"tabletitle\">$string[cercaip_title]</span></td></tr>".
  "<tr><td bgcolor=$style[table_bgcolor]><center><span class=\"tabletextA\">$string[cercaip_filtra_from]</span>&nbsp<input type=\"hidden\" name=\"action\" value=\"ip\"><input type=\"text\" name=\"from\" value=\"$from\"><span class=\"tabletextA\"> $string[cercaip_filtra_to]</span>&nbsp<input type=\"text\" name=\"to\" value=\"$to\"></center></td></tr>".
  "<tr><td bgcolor=$style[table_bgcolor]><center><input type=\"submit\" value=\"$string[cercaip_submit]\"></center></td></tr>".
  "</table>".
  "</form>";
return($return);
}
