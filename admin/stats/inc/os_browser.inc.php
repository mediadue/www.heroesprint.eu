...
$debug=(isset($_GET['debug']) ? addslashes($_GET['debug']) : 0);
...
/*** ORIGINALE: if($filter_number==='000') $filter_number='111'; ***/
...
//(Array)$data_os:
//query: os, sum of hits, sum of visits, grouped by os, ordered by sum of hits descending
//array format:
//Array(
//        Array((string)os,(int)hits,(int)visits),
//        ...
//);

//(array)$data_bw:
//query: bw, sum of hits, sum of visits, grouped by bw, 0=Browser or 1=Spider or 2=Grabber ordered by sum of hits descending
//array format:
//Array(
//        Array((string)bw,(int)sumhits,(int)sumvisits,(int)typeofbrowser);
//        ...
//);
...
//(array)$processed_os:
//Array(
//        Array((int)hits,(int)visits,(string)name),
//        ...
//);

//$processed_bw:
//Array(
//        Array((int)hits,(int)visits,(string)name,(int)typeofbrowser),
//        ...
//);
...
/*** ELIMINATO RAGGRUPPAMENTO PER MACROCATEGORIA
        else if($group===1){
                $bwnames=Array();
                $count=0;
                include('def/bw.dat');
                for($i=0,$tot=count($bw_def);$i<$tot;++$i){
                        $bwmacro[str_replace(' ','_',$bw_def[$i][0])]=$bw_def[$i][3];
                }
                ...
                for($i=0,$tot=count($data_bw);$i<$tot;++$i){
                        ...
                        $tmp=strrpos($bw_name,' ');
                        if($tmp===false) $tmpname=$bw_name;
                        else{
                            if(is_numeric($bw_name{$tmp+1})) $tmpname=substr($bw_name,0,$tmp);
                            else $tmpname=$bw_name;
                        }
                        $tmpname=$bwmacro[str_replace(' ','_',$tmpname)];
                        ...
                        if(isSet($bwnames[$tmpname])){
                                ...
                                $processed_bw[]=Array($bw_hits,$bw_visits,$tmpname,$typeofbw);
                                $bwnames[$tmpname]=$count;
                                ++$count;
                        }
                }
...
//(array)$output_os:
//Array(
//        Array((string)name,(int)hits,(int)visits,(float)percenthits,(float)percentvisits,(int)hitsbarlength,(int)visitsbarlength,(string)pictureurl),
//        ...
//);
...
/*** ELIMINATO RAGGRUPPAMENTO PER MACROCATEGORIA
        if($group!==2) $return.="<tr><td><span class=\"testo\"><a href=\"admin.php?action=os_browser&group=2{$filter_params}".($mode===0 ? "&mode=0&selected_month=$selected_month&selected_year=$selected_year" : '&mode=1')."\"><img src=templates/$option[template]/images/icon_changevis.gif border=\"0\" align=\"absmiddle\" hspace=\"1\" vspace=\"1\"><span class='testo'> $string[os_mode_2]</span></a></td></tr>";
***/
        $return.="</table>";
        if($modulo[11]){
                $return.=
