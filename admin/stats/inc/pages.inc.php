...
$mode_2=(isset($_GET['mode_2']) ? (int)$_GET['mode_2'] : 1);
...
$tables=Array('pagina'=>'data','hits'=>'realhits','in'=>'lev_1','out'=>'outs','io'=>'io');
...
do{
        ...
        $data_pages=Array();
        $res=sql_query("SELECT data,$singlehits,lev_1,outs,titlePage,(lev_1-outs) as io FROM $option[prefix]_pages $qFilter $filterZeroHits ORDER BY $sortBy $orderBy LIMIT $start,$recordPerPage");
        if(mysql_num_rows($res)<1) break;
        while($row=mysql_fetch_row($res)){
                list($pages_data,$pages_hits,$pages_lev_1,$pages_outs,$pages_titlePage,$pages_io)=$row;
                $data_pages[]=Array($pages_data,(int)$pages_hits,(int)$pages_lev_1,(int)$pages_outs,stripslashes(trim($pages_titlePage)),(int)$pages_io);
        }
...
if($dataFound){
        ...
        for($i=0,$tot=count($data_pages);$i<$tot;++$i){
                list($url,$hits,$ins,$outs,$pageTitle,$io)=$data_pages[$i];
                ...
                $output_pages[]=Array($url,$rawurl,$hits,$ins,$outs,$io,$percent);
        }
        unset($data_pages);
}
...
if($dataFound){
        ...
        for($i=0,$tot=count($output_pages);$i<$tot;++$i){
                list($url,$rawurl,$hits,$ins,$outs,$io,$percent)=$output_pages[$i];
