...
'0'.
($_GET['show_sp']=='1' ? '1' : '0');
if($filter_number==='000') $filter_number='100';
        ...
        for($i=0,$tot=count($visitors_id);$i<$tot;++$i){
                ...
                while($row=mysql_fetch_row($res)){
                        ...
                        if($details_titlePage==='?' && $option['page_title'] && $option['refresh_page_title']==0){
                                ...
                                list($dummy,$id)=explode('|',$page);
                                $res=sql_query("SELECT nome FROM $option[prefix]_clicks WHERE id='$id' LIMIT 1");
                                if(mysql_num_rows($tmpRes)>0){
                                        list($name)=mysql_fetch_row($tmpRes);
                                        $details_titlePage.=$details_currentPage.'|'.stripslashes($name);
                                }
                        ...
                        if($tmpRow===NULL) $tmpRow=Array($details_ip,$details_host,$details_os,$details_bw,$details_language,$details_referer,$details_reso,$details_colo,$domain,$returns,$last_return);
                }
                $tmpRow[]=$tmpPages;
...
/*
                        //isp
                        $isp=null;
                        if(in_array($domain,$foundIsps)){
                                $tmp=getIP($ip,148,$location.$domain."/$domain.db",$location.$domain."/$domain.idx",127);
                                if($tmp!=='unknown'){
                                        list($ispConnType,$ispName,$ispDesc)=explode('|',rtrim($tmp));
                                        $ispConnType=$arrayTypeConn[(int)$ispConnType];
                                        $ispDesc=(empty($ispDesc) ? null : $ispDesc);
                                        $isp=Array($ispConnType,$ispName,$ispDesc);
                                }
                        }
*/
                        //dottedIP
...
if($dataFound){
        ...
        for($i=0,$tot=count($processed_details);$i<$tot;++$i){
                ...
                if($bw===NULL) $bw='?';
                if($os===NULL) $os='?';
                if($isSpiderGrabber) $screenInfo='N/A';
                else{
                        if($resolution===null && $color===null) $screenInfo='?';
                        else{
                ...
                if($referer!==NULL){
                        if(count($referer)===1) $referer=formaturl($referer,'',80,50,-25);
                        else{
...
/***            "<a href=\"".($option['ext_whois']==='' ? "javascript:whois('whois.php?IP=$dottedIP');" : str_replace('%IP%',$dottedIP,$option['ext_whois']))."\" target=\"_BLANK\">$dottedIP</a>". ***/
                ...
                if($referer!==NULL){
                        $table.="\n\t\t\t<tr>";
                        if(count($referer)===1) $table.="<td bgcolor=$style[table_title_bgcolor] width=\"5%\" nowrap><span class=\"tabletitle\"><center>&nbsp;$string[details_referer]&nbsp;</center></span></td><td bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$referer</span></td></tr>";
                        else{
...
***/
        if($pageNumber>1){
                ...
                pag_bar('admin.php?action=details&mode='.$mode.$filter_params,$currentPage,$pageNumber,$recordPerPage).
                "</td></tr>".
                "<tr><td height=\"1\"bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
                "</table>";
        }
...
function check_details($time,$page,$title,$mode){
        ...
        if($title==='?' && $option['page_title'] && $option['refresh_page_title']==0){
                $res=sql_query("SELECT titlePage FROM $option[prefix]_pages WHERE data='$page' AND titlePage!='?' LIMIT 1");
                if(mysql_affected_rows()>0) list($title)=mysql_fetch_row($res);
        }
        else{
                $tmp=substr($page,0,3);
                if($tmp==='dwn'){
                        list($dummy,$id)=explode('|',$page);
                        $res=sql_query("SELECT nome FROM $option[prefix]_downloads WHERE id='$id' LIMIT 1");
                ...
                else $return.=formaturl($page,$title,80,50,-25,$title,$mode); // VISUALIZZO IL TITOLO DELLE PAGINE SE PRESENTE E SE ATTIVO
        }
        $return.="</span></td></tr>";
        return($return);
}
/*
function check_details($getTitles,$page,$title){
        global $option,$db,$string;

        if($title==='?'){
                if($getTitles){
                        $res=sql_query("SELECT titlePage FROM $option[prefix]_pages WHERE data='$page' AND titlePage!='?' LIMIT 1");
                        if(mysql_num_rows()>0) list($title)=mysql_fetch_row($res);
                }
        }
        else{
                $tmp=substr($page,0,3);
