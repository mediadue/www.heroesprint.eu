...
if(isset($_POST['sel_mese'])) $sel_mese=addslashes($_POST['sel_mese']);
else{ if(isset($_GET['sel_mese'])) $sel_mese=addslashes($_GET['sel_mese']); else $sel_mese=$mese; }
if(isset($_POST['sel_anno'])) $sel_anno=addslashes($_POST['sel_anno']);
else{ if(isset($_GET['sel_anno'])) $sel_anno=addslashes($_GET['sel_anno']); else $sel_anno=$anno; }
/*** ORIGINALE
...
***/
     if(isset($_GET['mode'])) $mode=addslashes($_GET['mode']); else if($modulo[5]<2) $mode=0; else $mode=1;
...
for($i=0;$i<24;++$i) $lista_accessi[$i]=$lista_visite[$i]=0;

if(strlen("$sel_mese")<2) $sel_mese='0'.$sel_mese;
$clause=($mode==0 ? "WHERE mese='$sel_anno-$sel_mese'" : '');

// Titolo pagina (riportata anche nell'admin)
if($mode==0) $phpstats_title=str_replace(Array('%MESE%','%ANNO%'),Array(formatmount($sel_mese),$sel_anno),$string['hourly_title_2']);
else $phpstats_title=$string['hourly_title'];
    switch($mode_2){
         case 0: $phpstats_title.=$string['hourly_string_mode_0']; break;
         case 1: $phpstats_title.=$string['hourly_string_mode_1']; break;
         case 2: $phpstats_title.=$string['hourly_string_mode_2']; break;
    }
//
...
"<tr><td bgcolor=$style[table_bgcolor] colspan=\"26\" nowrap><span class=\"tabletextA\"><center><img src=\"templates/$option[template]/images/style_bar_1.gif\" width=\"7\" height=\"7\"> $string[hits] <img src=\"templates/$option[template]/images/style_bar_2.gif\" width=\"7\" height=\"7\"> $string[visite]</span></center></td></tr>".
//"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"26\" nowrap></td></tr>".
'</table>';
...
if($modulo[5]==2)
{
  ...
  if($mode==0)
    ...
    $return.=
    "<form action='./admin.php?action=hourly' method='POST' name=form1><span class=\"tabletextA\">$string[calendar_view]</span>".
    ...
    "<br><br><a href=\"admin.php?action=hourly&mode=1\"><img src=templates/$option[template]/images/icon_change.gif border=\"0\" align=\"absmiddle\" hspace=\"1\" vspace=\"1\"><span class='testo'> $string[os_vis_glob]</span></a>".
    '</FORM>';
    }
  else $return.="<a href=\"admin.php?action=hourly&mode=0\"><img src=templates/$option[template]/images/icon_change.gif border=\"0\" align=\"absmiddle\" hspace=\"1\" vspace=\"1\"><span class='testo'> $string[os_vis_mens]</span></a>";
  $return.="</center>";
}
if($modulo[11]){
        // SELEZIONE MODALITA'
