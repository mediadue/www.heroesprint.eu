...
if(!defined('IN_PHPSTATS')) die('Php-Stats internal file.');
      ...
      if(isset($_GET['mounth1'])) $mounth1=addslashes($_GET['mounth1']); else $mounth1='last';
      if(isset($_GET['year1'])) $year1=addslashes($_GET['year1']); else $year1='last';
      if(isset($_GET['mounth2'])) $mounth2=addslashes($_GET['mounth2']); else $mounth2='last';
      if(isset($_GET['year2'])) $year2=addslashes($_GET['year2']); else $year2='last';
      if(isset($_GET['view_graphics'])) $view_graphics=addslashes($_GET['view_graphics']); else $view_graphics=0; 
/*** ORIGINALE
...
$return=
"<br><br><center><form action='./admin.php' method='GET' name=form1><span class='testo'>$string[compare_comp]</span>".
'<input name="action" type="hidden" value="compare">'.
'<SELECT name=mounth1>';
for($i=1;$i<=12;++$i) $return.="<OPTION value='$i'".($mounth1_sel==$i ? ' SELECTED' : '').'>'.$varie['mounts'][$i-1].'</OPTION>';
$return.="</SELECT><SELECT name=year1>";
for($i=$anno_y;$i<=$year_now;++$i) $return.="<OPTION value='$i'".($year1_sel==$i ? ' SELECTED' : '').">$i</OPTION>";
$return.="</SELECT><span class='testo'> $string[compare_with] </span><SELECT name=mounth2>";
for($i=1;$i<=12;++$i) $return.="<OPTION value='$i'".($mounth2_sel==$i ? ' SELECTED' : '').'>'.$varie['mounts'][$i-1].'</OPTION>';
$return.='</SELECT><SELECT name=year2>';
for($i=$anno_y;$i<=$year_now;++$i) $return.="<OPTION value='$i'".($year2_sel==$i ? ' SELECTED' : '').">$i</OPTION>";
$return.=
"</SELECT><input name=\"view_graphics\" type=\"hidden\" value=\"1\">".
...
if($view_graphics==1)
    ...
    if(checkdate($mounth1,$i,$year1))
      ...
      switch($mode){
           case 0:
            ...
            if($row['hits']>$max_accessi) $max_accessi=$row['hits'];
            if($row['visits']>$max_visite) $max_visite=$row['visits'];
            break;
      case 1:
            $lista_accessi_1[$i]=$row['hits']-$row['no_count_hits'];
            $lista_visite_1[$i]=$row['visits']-$row['no_count_visits'];
            if(($row['hits']-$row['no_count_hits'])>$max_accessi) $max_accessi=($row['hits']-$row['no_count_hits']);
            if(($row['visits']-$row['no_count_visits'])>$max_visite) $max_visite=($row['visits']-$row['no_count_visits']);
            break;
      case 2:
            $lista_accessi_1[$i]=$row['no_count_hits'];
            $lista_visite_1[$i]=$row['no_count_visits'];
            if($row['no_count_hits']>$max_accessi) $max_accessi=$row['no_count_hits'];
            if($row['no_count_visits']>$max_visite) $max_visite=$row['no_count_visits'];
            break;
     }
      }
      }
  ...
  "</table></td>";
  for($i=1;$i<=31;++$i) $return.="<td height=\"200\" width=\"15\" valign=\"bottom\" align=\"center\" background=\"templates/$option[template]/images/table_grid.gif\"><img src=\"templates/$option[template]/images/style_bar_3.gif\"\" width=\"5\" height=\"".($lista_accessi_1[$i]/$max_v*187)."\"  title=\"$lista_accessi_1[$i]\"><img src=\"templates/$option[template]/images/style_bar_4.gif\"\" width=\"5\" height=\"".($lista_accessi_2[$i]/$max_v*187)."\" title=\"$lista_accessi_2[$i]\"></td>";
  $return.=
  "<td height=\"200\" width=\"1\" valign=\"bottom\" align=\"center\" background=\"templates/$option[template]/images/table_grid.gif\"></td>".
  "</td></tr><tr><td><img src=\"templates/$option[template]/images/style_bar_1.gif\" width=\"7\" height=\"7\"></td>";
  for($i=1;$i<=31;++$i)
    {
    list($giorno,$weekday)=explode('-',date('d-w',mktime(0,0,0,$mounth1,$i,$year1)));
    if(checkdate($mounth1,$i,$year1)) $return.="<td><span class=\"".($weekday==0 ? 'tabletextB' : 'tabletextA')."\">$giorno</span></td>";
    }
  ...
  for($i=1;$i<=31;++$i)
    ...
    if(checkdate($mounth2,$i,$year2)) $return.="<td><span class=\"".($weekday==0 ? 'tabletextB' : 'tabletextA')."\">$giorno</span></td>";
    }
  $return.=
  "</tr>".
  ...
  $max_v=$tmp*6;
  $return.=
  "<br><span class=\"pagetitle\">$string[compare_visits]<br><br></span>".
  "<table bgcolor=$style[table_bgcolor] border=\"0\" cellpadding=\"1\" cellspacing=\"1\" align=\"center\">".
  "<tr><td><table bgcolor=$style[table_bgcolor] border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\">".
  "<tr><td height=\"30\"><span class=\"testo\">".($tmp*5)."</span></td></tr>".
  "<tr><td height=\"30\"><span class=\"testo\">".($tmp*4)."</span></td></tr>".
  ...
  "<tr><td height=\"30\"><span class=\"testo\">".($tmp*1)."</span></td></tr>".
  "</table></td>";
  for($i=1;$i<=31;++$i) $return.="<td height=\"200\" width=\"15\" valign=\"bottom\" align=\"center\" background=\"templates/$option[template]/images/table_grid.gif\"><img src=\"templates/$option[template]/images/style_bar_3.gif\"\" width=\"5\" height=\"".($lista_visite_1[$i]/$max_v*187)."\"  title=\"$lista_visite_1[$i]\"><img src=\"templates/$option[template]/images/style_bar_4.gif\"\" width=\"5\" height=\"".($lista_visite_2[$i]/$max_v*187)."\" title=\"$lista_visite_2[$i]\"></td>";
  $return.=
  "<td height=\"200\" width=\"1\" valign=\"bottom\" align=\"center\" background=\"templates/$option[template]/images/table_grid.gif\"></td>".
  "</td></tr><tr><td><img src=\"templates/$option[template]/images/style_bar_1.gif\" width=\"7\" height=\"7\"></td>";
  for($i=1;$i<=31;++$i)
  ...
  //////////////////////////////////////
  $return.=
  "<br><span class=\"pagetitle\">$string[compare_both]<br><br></span>".
  "<table border=\"0\" $style[table_header] width=\"90%\">".
  "<tr>".
  "<td bgcolor=$style[table_title_bgcolor] colspan=\"2\" nowrap><span class=\"tabletitle\"><center>$string[compare_hits]</center></span></td>".
  "<td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center>".$varie['mounts_1'][$mounth1-1]." $year1</center></span></td>".
  "<td bgcolor=$style[table_title_bgcolor] nowrap><span class=\"tabletitle\"><center>".$varie['mounts_1'][$mounth2-1]." $year2</center></span></td>".
  "<td bgcolor=$style[table_title_bgcolor] colspan=\"2\" nowrap><span class=\"tabletitle\"><center>$string[compare_visites]</center></span></td>".
  "</tr>";
  for($i=0;$i<=31;++$i)
    {
    ...
    if((checkdate($mounth1,$i,$year1))|(checkdate($mounth2,$i,$year2)))
      ...
      if(checkdate($mounth1,$i,$year1)) $return.="<td bgcolor=$style[table_bgcolor] align=\"center\"><span class=\"".($weekday==0 ? 'tabletextB' : 'tabletextA')."\">$giorno</span></td>";
    ...
    if((checkdate($mounth1,$i,$year1))|(checkdate($mounth2,$i,$year2)))
      ...
      if(checkdate($mounth2,$i,$year2)) $return.="<td bgcolor=$style[table_bgcolor] align=\"center\"><span class=\"".($weekday==0 ? 'tabletextB' : 'tabletextA')."\">$giorno</span></td>";
      else $return.="<td bgcolor=$style[table_bgcolor]></td>";
  ...
  "<td bgcolor=$style[table_bgcolor] align=\"center\" colspan=\"2\" nowrap><span class=\"tabletextA\"><b>$string[compare_total]</b></span></td>".
