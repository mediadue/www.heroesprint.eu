...
// ESECUZIONE QUERY //
function sql_query($query,$logerror=TRUE) {
  global $option,$db,$return,$error;
  ++$GLOBALS['totalqueries'];
  $query = str_replace('{prefix}', $option['prefix'], $query);
  $return=mysql_query($query);
  if($return==false && $logerror)
    {
    $error['debug_level']=1;
...
function ver_comp($first,$second)
  {
  $first=explode('.',$first);
  $second=explode('.',$second);
  for($i=0,$tot=max(count($first),count($second));$i<$tot;$i++)
    {
    if(!isSet($first[$i])) return -1;
...
***/
  ...
  if(mysql_affected_rows()<1) sql_query("INSERT INTO $option[prefix]_daily VALUES('$Cgiorno','$Chits','$Cvisits'".(($spider_agent) ? ",'$Chits','$Cvisits'" : ",'0','0'").")");
/*** ***/
  }
