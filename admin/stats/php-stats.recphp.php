...
//	include(__PHP_STATS_PATH__.'option/php-stats_mode.php');
//	if(!isset($NowritableServer))
//		$NowritableServer=1;

	// VARIABILI ESTERNE
	                        /***if(!isset($_COOKIE)) $_COOKIE=$HTTP_COOKIE_VARS;	deprecated */
	  if(isset($_COOKIE['php_stats_esclusion'])) $php_stats_esclusion=$_COOKIE['php_stats_esclusion']; else $php_stats_esclusion='';
	                        /***if(!isset($_SERVER)) $_SERVER=$HTTP_SERVER_VARS;	deprecated */
...
/*
	if($NowritableServer===1)	// Legge le opzioni dal database mySQL
	{
   		// Inizializzo l'oggetto
//   		$php_stats_rec = new php_stats_recFunction;
   		$php_stats_rec->php_stats_setvariables($option);
// 		$php_stats_rec->php_stats_db_connect(); // CONNESSIONE MYSQL
   		$result=$php_stats_rec->php_stats_sql_query("SELECT option_name,option_value FROM $option[prefix]_options");
   		while($row=mysql_fetch_row($result))
   			$option2[$row[0]]=$row[1]; // DA SCEGLIERE
...
*/
	...
	{
		$php_stats_rec->php_stats_sql_query("SELECT ip FROM $option[prefix]_ip_spam WHERE ip=$nip");
		if(mysql_affected_rows()>0)
		{
/*			if($option['logerrors'])
				file_put_contents(__PHP_STATS_PATH__.'php-stats.log', date('d/m/y H:i')." ** IP spam: ".long2ip($ip)." ($hostname)\n", FILE_APPEND);*/
        	$php_stats_rec->php_stats_logerrors(date('d/m/y H:i')." ** IP spam: ".long2ip($ip)." ($hostname)");
			return(0);
		}
	}
...
//		$php_stats_rec->php_stats_db_connect(); // CONNESSIONE MYSQL
	...
	$result=$php_stats_rec->php_stats_sql_query("SELECT data,lastpage,user_id,visitor_id,reso,colo,os,bw,tld,giorno,level FROM $option[prefix]_cache WHERE user_id='$ip' LIMIT 1");
	if(mysql_affected_rows()>0)
...
/*** */		$last_page_url = mysql_real_escape_string($last_page_url);
	...
	{ // VERIFICO IL RICONOSCIMENTO CONTINUO
	...
	if ($reffer!='')
   	{
   		...
   		if($php_stats_rec->php_stats_is_internal($tmpreffer)===FALSE && ($do_insert || $option['full_recn']))
	...
	/*** */ 		$query = mysql_real_escape_string($query);
        			...
        			if($modulo[4]==2) $clause.=" AND mese='$mese_oggi'";
...
// SCRIVO I DETTAGLI
	if($modulo[0])
	{
  		if((!$option['../stats/refresh_page_title']) && $modulo[3] && $loaded!=='?' && $titleExist===true)
  		{
...
//  				$php_stats_rec->php_stats_sql_query("UPDATE $option[prefix]_options SET option_value='$online|$date' WHERE option_name='instat_max_online'");
  		}
	}
