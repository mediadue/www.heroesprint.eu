...
if($num_totale>0) {
  ...
  while($row=mysql_fetch_array($result))
    ...
    $count=0;
    list($details_id)=$row;
    if (($data_start!='')&&($data_end!='')) $result2=sql_query("SELECT ip,host,os,bw,lang,date,referer,currentPage,reso,colo,titlePage,tld FROM $option[prefix]_details WHERE visitor_id='$details_id' AND date BETWEEN $data_start AND $data_end ORDER BY date ASC");
    else $result2=sql_query("SELECT ip,host,os,bw,lang,date,referer,currentPage,reso,colo,titlePage,tld FROM $option[prefix]_details WHERE visitor_id='$details_id' ORDER BY date ASC");
    $visitor_pages=mysql_num_rows($result2);
    while($row2=mysql_fetch_array($result2))
      {
      list($details_ip,$details_host,$details_os,$details_bw,$details_language,$details_date,$details_referer,$details_currentPage,$details_reso,$details_colo,$details_titlePage,$domain)=$row2;
      if($count==0)
        {
        if($closed==0)
          {
        ...
        $dottedIP=long2ip($details_ip);

                $data_write.=
                "<td bgcolor=\"#CCCCCC\"><center>$reso</center></td>".
                "<td bgcolor=\"#999999\"><center>$dottedIP</center></td>".
                "</tr>";
        write_file_dump();
                $count=0;
                $table='';
        ...
        if($option['ip-zone'] && $modulo[7]){
          ...
          if($isp_descr!='') $table.="<tr><td bgcolor=\"#999999\"><center>$string[details_descr]</center></span></td><td colspan=\"3\" bgcolor=\"#CCCCCC\">$isp_descr</td></tr>";
                ...
                // HOST
                 if($details_host!='' && $details_host!=$dottedIP) // Non visualizzo se HOST è vuoto
                  {
                  $table.=
                  "<tr>".
