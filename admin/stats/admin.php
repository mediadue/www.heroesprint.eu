...
//           if(isset($_POST['change_mode'])) $change_mode=addslashes($_POST['change_mode']); else $change_mode='';

$GLOBALS['totalqueries']=0; // Contatore Query - Counter Queries

// Rilevo il tipo di modalità utilizzata: no_write o write - Check no_write or write modality
//include('option/php-stats_mode.php');
//if(!isset($NowritableServer))
//	$NowritableServer=1;

// VERIFICO IL CAMBIO DI MODALITA'
$createOption=FALSE;
/*
if($change_mode==1)
{
  chmod('option/php-stats_mode.php', 0666);
  if($NowritableServer==1)
  {
    $writeServer=FALSE;
    $writeServer=touch('option/php-stats-options.php');
    $WriteType=($writeServer ? '$NowritableServer=0;' : FALSE);
  }
  else
  	$WriteType='$NowritableServer=1;';
...
//		die('<b>ERRORE</b>: File config.php non accessibile.');
//}
...
if($NowritableServer===1)
{
  ...
  while($row=mysql_fetch_row($result))
  	$option[$row[0]]=$row[1];
  eval($option['php-stats-options']);
}
*/
if($createOption===TRUE)
{
//  $NowritableServer=($NowritableServer==1 ? 0 : 1);
  create_option_file();
}
...
if(!$option['clear_cache'])
	{ // Controllo se non si è forzato il riconoscimento continuo - Check for continuous forced recognition
...
if($cache_recn==1 && (user_is_logged_in() || (!$option['use_pass'] && in_array($action,$unlockedPages))))
{
	...
	if($do==0)
	{
	  	// VISUALIZZO MESSAGGIO DI ATTESA - SHOE WAIT MESSAGE
	...
	*/
	...
	//	if ($is_loged_in && (time()-$option['inadm_last_update'])>(432000*2) && !$option['inadm_upd_available'] && $option['check_new_version'])
		// Check nuove versioni (ogni 5 giorni)
		if ($is_loged_in && (time()-$option['inadm_last_update'])>(5*24*3600) /*&& !$option['inadm_upd_available']*/ && $option['check_new_version'])
		{
			$update = @file_get_contents('http://robertobizzarri.altervista.org/php-stats/phpstats_ver_check.php?url='.trim($option['script_url']).'&ver='.trim($option['phpstats_ver']));
		...
		if ($option['inadm_upd_available'] || $update_available)
		{
			$tips=
			"<br>\n<table $style[table_header] width=\"95%\">".
			"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
...
/*** Controllo validità dei file di browscap */
	if ( filesize('browscap/browscap.ini') < 50000 )
	{
		...
		"<br>\n<table $style[table_header] width=\"95%\">".
		"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
		"<tr bgcolor=\"$style[table_tips_bgcolor]\"><td width=\"95%\" bgcolor=\"$style[table_tips_bgcolor]\"><span class=\"tabletextA\"><img src=\"templates/$option[template]/images/icon_tips.gif\" align=\"absmiddle\" border=\"0\">$string[tips_bcap_ini]</span></td></tr>".
		"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
		"</table>";
		$action=$action.$tips;
	}
	else if ( file_exists('browscap/cache.php') && filesize('browscap/cache.php') < 50000 )
	{
		...
		"<br>\n<table $style[table_header] width=\"95%\">".
		"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
		"<tr bgcolor=\"$style[table_tips_bgcolor]\"><td width=\"95%\" bgcolor=\"$style[table_tips_bgcolor]\"><span class=\"tabletextA\"><img src=\"templates/$option[template]/images/icon_tips.gif\" align=\"absmiddle\" border=\"0\">$string[tips_bcap_cache]</span></td></tr>".
		"<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] nowrap></td></tr>".
		"</table>";
		$action=$action.$tips;
	}
